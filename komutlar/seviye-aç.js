...
exports.run = async(client, message, args) => { 
    ...
    if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`<a:yanls:731299680285098065> Bu komutu kullanabilmek için "\`Yönetici\`" yetkisine sahip olmalısın.`);
  
 
  let ufuk = await db.fetch(`seviyeacik_${message.guild.id}`)
  
  if(ufuk) return message.reply('Bu tuhaf! Anlaşılan seviye sistemi zaten aktif edilmiş.. \n Bunu mu arıyorsun? `s+seviye-kapat`')
